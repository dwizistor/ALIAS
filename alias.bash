...
chrootvars(){
    ...
    commands=(
        ...
        "echo 'net.ipv4.tcp_fastopen = 3' | tee -a usr/lib/sysctl.d/70-cachyos-settings.conf"
        "echo 'net.ipv4.tcp_timestamps = 0' | tee -a usr/lib/sysctl.d/70-cachyos-settings.conf"
        "echo 'kernel.split_lock_mitigate=0' | tee -a usr/lib/sysctl.d/70-cachyos-settings.conf"
        "cp -rvf ./etc/. /etc"
        "cp -rvf ./usr/. /usr"
        ...
        ########################################################
        "> Enabling TCP BBR"
        "touch /etc/modules-load.d/bbr.conf"
        "echo 'tcp_bbr' | tee -a /etc/modules-load.d/bbr.conf"
        "echo 'net.core.default_qdisc = fq' | tee -a usr/lib/sysctl.d/70-cachyos-settings.conf"
        "echo 'net.ipv4.tcp_congestion_control = bbr' | tee -a usr/lib/sysctl.d/70-cachyos-settings.conf"
        ########################################################
        "> Configuring hosts"
